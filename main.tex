...
\subsection{Versuchsdurchführung}
Für Versuchsteil a) wurde eine Kaliumnitrat-Lösung mit wenigen Tropfen Schwefelsäure angesäuert. Diese Lösung wurde eine Eisensulfat-Lösung vermischt und mit konzentrierter Schwefelsäure unterschichtet.\\

In Versuchsteil b) wurde eine Eisensulfat-Lösung mit Kaliumnitrat und 2 Spatelspitzen Natriumhydroxid versetzt. Das Becherglas wurde mit einem Uhrglas abgedeckt, an dem ein Stück angefeuchtetes Indikatorpapier befestigt war.



\subsection{Beobachtungen}
In Versuchsteil a) bildete sich ein brauner Ring an der Phasengrenze von Schwefelsäure und Eisensulfatlösung.\\

In Versuchsteil b) fiel ein rotbrauner Feststoff aus der Lösung aus und das pH-Papier färbte sich nach einiger Zeit blaugrün.



\subsection{Auswertung}

Im Sauren besteht die Reaktion von Eisen(II)-Ionen mit Nitrat-Ionen aus 2 Teilreaktionen. In der Ersten wird das Eisen oxidiert und das Nitrat zu Stickstoffmonoxid reduziert.
\begin{align*}
...
Im zweiten Schritt reagiert Stickstoffmonoxid mit überschüssigen $\ce{Fe^{2+}}$ zu einem Pentaaquanitrosyleisen(II) Komplex:
\begin{equation}
    \ce{[Fe(H2O)6]^{2+} + NO -> [Fe(H2O)_5NO]^{2+} + H2O}
\end{equation}

Im basischen reagiert Eisensulfat mit Kaliumnitrat zu Ammoniak. Dabei wird Eisen ebenfalls oxidiert, der Stickstoff aber deutlich stärker reduziert. Grund hierfür ist, dass Eisen als Eisenhydroxid ausfällt und so nicht für die Redoxreaktion zur Verfügung steht.
...
Das aufsteigende Ammoniak reagiert mit dem Wasser im pH-Papier:
\begin{equation}
    \ce{NH3_{\text{(g)}} + H2O_{\text{(l)}} -> NH4+_{\text{(aq)}} + OH^-_{\text{(aq)}}}
\end{equation}
Die entstehenden Hydroxidionen färben das pH-Papier blau.\\

Reaktion von Eisenionen mit Hydroxid-Ionen:
\begin{equation}
    \ce{Fe^{2+}_{(aq)} + 2OH^{-}_{(aq)} -> Fe(OH)2_{\text{(s)}} v }
\end{equation}

Das Redoxpotential ${{\text{Fe(OH)}}_3}_{\text{(aq)}} + e^- \rightleftharpoons {{\text{Fe(OH)}}_2}_{\text{(aq)}} + \text{OH}^{-}_{\text{(aq)}}$ liegt bei -0,56 V\textsuperscript{\cite{redox}}, das von $\text{Fe}^{3+}_{\text{(aq)}} + e^- \rightleftharpoons \text{Fe}^{2+}_{\text{(aq)}}$ bei +0,77 V\textsuperscript{\cite{allg_ac_buch_riedel}}. Im alkalischen ist Eisen deutlich unedler, weshalb es auch besser oxidiert werden kann. Dadurch ist es Stickstoff möglich, weiter reduziert zu werden als im sauren. 




\subsection{Fehlerbetrachtung}
Fehler in den Nachweisen entstanden durch zu geringe Konzentrationen von Eisensulfat oder Kaliumnitrat. Dadurch kann die Reaktion nicht ideal ablaufen und die braune Farbe übersehen werden. Auch schlechtes Durchmischen der Lösungen führt zu Fehlern.
...
\subsection{Aufgabenstellung}
Die Aufgabe war es, die Stoffmenge von Kupferionen iodometrisch zu bestimmen.


\subsection{Versuchsdurchführung}
Der Analysekolben wurde bis zur Eichmarke aufgefüllt und geschüttelt. Mit einer Vollpipette wurden 25 mL in einen Erlenmeyerkolben titriert. Der Erlenmeyerkolben wurde etwas mit demin. Wasser aufgefüllt und mit einer Tropfpipette konzentrierter Essigsäure versetzt. In einem Reagenzglas wurde Kaliumiodid gelöst und in den Kolben gegeben. Zusätzlich wurde eine Stärkelösung über einem Teclubrenner gekocht. Nachdem sich die gesamte Stärke gelöst hatte, wurde auch diese Lösung in den Kolben gegeben. Die Bürette wurde mit einer 0,1 molaren  Natriumthiosulfat-Lösung befüllt und die Probe bis zum Farbumschlag titriert.   


\subsection{Beobachtungen}
Nach der Zugabe von Iodid-Ionen färbte sich die hellblaue Kupferlösung gelborange und es bildete sich ein Niederschlag. Nach der Zugabe der Stärke färbte sich die Lösung dunkelblau. Am Äquivalenzpunkt lag die Lösung farblos vor.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Bilder/Thiosulfat.jpg}
        \caption{Strukturformel des Thiosulfat-Ions}
        \label{fig: Thiosulfat}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Bilder/Tetrathionat.jpg}
...
\begin{align*}
    ...
    n(\ce{Cu^{2+}}) &= 0,1\,\frac{{\text{mol}}}{{\text{L}}} \cdot 9,5\,\text{mL}\\[2mm]
    n(\ce{Cu^{2+}}) &= 0,95\,\text{mmol}
\end{align*}
...
\subsection{Fehlerbetrachtung}
Mögliche Fehlerquellen sind eine leichte Verbrennung der Stärkelösung, Abweichungen im Verhältnis der Stärke und Iod in der Lösung, Ablesefehler an der Bürette und menschliche Fehler beim Ablesen der Farbe.\
Die prozentuale Abweichung der ermittelten Stoffmenge von der Sollstoffmenge von 3,88 m beträgt:
\begin{align*}
    \Delta n &= \left(1 - \frac{n_{\ce{Cu^-}}(\text{ist})}{n_{\ce{Cu^-}}(\text{soll})} \right) \cdot 100\%\\
    \Delta n &= \left( 1 - \frac{3,80\,\text{mmol}}{3,88\,\text{mmol}} \right) \cdot 100\%\\ 
    \Delta n &= 2,1\% 
\end{align*}
